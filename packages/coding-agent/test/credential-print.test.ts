...
describe("credential print commands", () => {
	test("prints a resolved API key", async () => {
		const runtime = await createRuntime(AuthStorage.inMemory({ deepseek: { type: "api_key", key: "test-api-key" } }));
	...
	test("refreshes an expired OAuth token before printing it", async () => {
		...
		expect(await storage.read("kimi-coding")).toMatchObject({ access: "fresh-test-token" });
	});
