...
describe("Anthropic temperature compatibility", () => {
	it("omits temperature for Claude Opus 4.7", async () => {
		const payload = await capturePayload(getModel("minimax", "MiniMax-M2.7"), { temperature: 0 });
	...
	it("omits default temperature for Claude Opus 4.7", async () => {
		const payload = await capturePayload(getModel("minimax", "MiniMax-M2.7"), { temperature: 1 });
